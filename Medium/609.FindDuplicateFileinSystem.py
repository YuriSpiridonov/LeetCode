"""
    ...
    (f1.txt, f2.txt ... fn.txt with content f1_content, f2_content ... 
                     fn_content, respectively) in directory root/d1/d2/.../dm. 
    ...
    2. You may assume the directory name, file name and file content only has 
       letters and digits, and the length of file content is in the range of [1,50].
    3. The number of files given is in the range of [1,20000].
    4. You may assume no files or directories share the same name in the same 
       directory.
...
#Runtime: 80 ms, faster than 97.25% of Python3 online submissions for Find Duplicate File in System.
...
class Solution:
    def findDuplicate(self, paths: List[str]) -> List[List[str]]:
        ...
        for path in paths:
            ...
            root = files[0]
            for file in files[1:]:
                filename, content = file.replace(r')', '').split(r'(')
                if content not in db:
                    db[content] = [root+'/'+filename]
                else:
                    db[content].append(root+'/'+filename)
        return [value for value in db.values() if len(value) > 1]
